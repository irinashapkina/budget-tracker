    ...
    <div class="group">
      <div class="status">
        <div class="info-c">
          ...
          <p>Целевая сумма:</p>
          <input type="number" v-model="formData.target_sum">
...
<script>
import {addTarget} from "@/services/api.js";
...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        const response = await addTarget(this.formData);
        this.$router.push('/target-list');
      } catch (error) {
        console.error('Ошибка при добавлении цели:', error.response.data);
      }
    }
  }
};
